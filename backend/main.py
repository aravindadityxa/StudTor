...
# Security
security = HTTPBearer()
app = FastAPI(title="StudTor", version="1.0.0")
...
# Initialize database and Local LLM client
init_db()
...
def chat(chat_data: ChatMessage, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        response = local_llm.generate_response(chat_data.message, chat_data.language or user.preferred_language)
...
def chat_public(chat_data: ChatMessage):
    try:
        response = local_llm.generate_response(chat_data.message, chat_data.language or "en")
...
@app.get("/")
def read_root():
    return {"message": "StudTor Backend is running"}

@app.get("/health")
def health_check():
    """Health check endpoint that includes Ollama status."""
    status_info = local_llm.get_status()
    return {
        "backend": "online",
        "ollama": "online" if status_info["ollama_running"] else "offline",
        "model_available": status_info["model_available"],
...
def ai_status():
    ...
    return {
        "provider": "ollama",
        "model": status_info["model"],
        "ready": status_info["ollama_running"] and status_info["model_available"],
        "ollama_url": status_info["ollama_url"],
    }
